...
readonly _ApiBot_SH_=1					# Inicialização
...
CreateLog()
{
	...
	for ((i=0; i < $1; i++)); do
		...
		fmt=${fmt//\{METHOD\}/${FUNCNAME[2]/main/ApiBot.getUpdates}}
...
MessageError()
{
	...
	# API - Erro interno gerado pela API do ApiBot.
	case $1 in
		TG)
...
{    
	...
	while :; do
		jq_obj=$(ApiBot.getUpdates --limit 100 --offset $(ApiBot.OffsetNext))
...
_eof
	...
	[[ $? -eq 0 ]] && {	
		...
		sed -i -r '/^\s*ApiBot.init\s/s/\s--?(s(ervice)?|u(ser)?\s+\w+)\b//g' "$1"
...
ApiBot.init()
{
	...
	[[ $_ApiBot_INIT_ ]] && MessageError API "$_ERR_BOT_ALREADY_INIT_"
				...
				-t|--token)
	   			...
	   			-m|--monitor)
					...
					monitor=true
	   				...
	   				;;
				-f|--flush)
					# Define a FLAG flush para o método 'ApiBot.getUpdates'. Se ativada, faz com que
	...
	ApiBot.getMe &>- || MessageError API "$_ERR_TOKEN_UNAUTHORIZED_" '[-t, --token]'
	...
	declare -gr _ApiBot_INIT_=1

    # ApiBot (FUNÇÕES)
	# Inicializa as funções para chamadas aos métodos da API do telegram.
	ApiBot.ListUpdates(){ echo ${!update_id[@]}; }
	ApiBot.TotalUpdates(){ echo ${#update_id[@]}; }
	ApiBot.OffsetEnd(){ local -i offset=${update_id[@]: -1}; echo $offset; }
	ApiBot.OffsetNext(){ echo $((${update_id[@]: -1}+1)); }
   	
	ApiBot.token() { echo "${_BOT_INFO_[0]}"; }
	ApiBot.id() { echo "${_BOT_INFO_[1]}"; }
	ApiBot.first_name() { echo "${_BOT_INFO_[2]}"; }
	ApiBot.username() { echo "${_BOT_INFO_[3]}"; }
	...
	ApiBot.setStickerPositionInSet()
	{
...
_EOF
	...
	ApiBot.editMessageLiveLocation()
	{
		local chat_id message_id inline_message_id
	...
	ApiBot.stopMessageLiveLocation()
	{
		local chat_id message_id inline_message_id reply_markup jq_obj
	...
	ApiBot.setChatAdministratorCustomTitle()
	{
	...
	ApiBot.manageRules()
	{
		...
		for ((i=0; i < _BOT_RULES_INDEX_; i++)); do
			...
			if [[ ${_BOT_RULES_[$i:user_status]} ]]; then
				case $_BOT_TYPE_RETURN_ in
					value)
						out=$(ApiBot.getChatMember 	--chat_id $u_message_chat_id \
					...
					json)
						...
						;;
					map)	
						ApiBot.getChatMember 	--chat_id $u_message_chat_id \
			...
			if [[ ${_BOT_RULES_[$i:bot_action]} ]]; then
				ApiBot.sendChatAction --chat_id $u_message_chat_id --action ${_BOT_RULES_[$i:bot_action]} &>/dev/null
			fi

			if [[ ${_BOT_RULES_[$i:bot_reply_message]} ]]; then
				ApiBot.sendMessage	--chat_id $u_message_chat_id 																\
			...
			for fwid in ${_BOT_RULES_[$i:bot_forward_message]//|/ }; do
				ApiBot.forwardMessage		--chat_id $fwid						\
	...
	eval set -- "$param"
		...
		# Se o método 'ApiBot.getUpdates' for invocado a partir de um subshell,
